...
TEST_PERF_TEST(
    ...
    CREATE_STRING(
        ...
        "- 100y AND now() AND measure_name = 'cpu_user' AND region = 'us-east-1' "
...
TEST_PERF_TEST(
    ...
    CREATE_STRING(
        ...
        "FROM perfdb_hcltps.perftable_hcltps WHERE time BETWEEN now() - 100y AND "
...
TEST_PERF_TEST(
 ...
 TEST_PERF_TEST(
     ...
     CREATE_STRING(
         ...
         "availability_zone, microservice_name, instance_name, "
         "AVG(measure_value::double) AS instance_avg_metric FROM "
         "perfdb_hcltps.perftable_hcltps WHERE time BETWEEN now() - 100y AND now() "
         "AND "
